...
const Sidebar = ({ professors }) => {
  ...
  return (
    // <div className="h-[2000px] shadow-lg text-black text-lg flex flex-col items-start w-96">
      ...
      >
         <Menu.Items className="absolute left-10 right-0 w-56 mt-2 origin-top-right bg-red-700 divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
              ...
              <Menu.Item key={prof}>
                {({ active }) => (
                  <button 
                    ...
                    className={`group flex items-center w-full px-2 py-2 text-sm ${active ? 'bg-red-500 text-white' : 'text-white hover:bg-red-400 hover:text-black'}`}
                 >
                  {prof}
                  </button>
                )}
